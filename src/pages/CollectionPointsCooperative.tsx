...
export function CollectionPointsCooperative() {
  ...
  useEffect(() => {
    const fetchCollectionPoints = async () => {
      setLoading(true);
      try {
        const response = await api.get('cooperatives/?skip=0&limit=100',
          {
        ...
        const fetchedPoints = response.data.map((item: any) => {
          const point: CollectionPoint = {
            id: item.id,
            name: `${item.corporate_name}`, // Nome genérico (não presente no dado original)
            type: 'Ponto de Descarte', // Tipo genérico (não presente no dado original)
            ...
            distance: currentLat && currentLng && item.latitude && item.longitude
              ...
              : 'Indisponível',
            materials: item.materials.map((m: any) => `${m}`),
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="container mx-auto px-4">
        <h1 className="text-3xl font-bold mb-8">Pontos de Descarte</h1>
            ...
            {points.map((point) => (
              <div key={point.id} className="bg-white rounded-lg shadow-md p-6">
                ...
                <div className="border-t pt-4">
                  <h4 className="text-sm font-semibold mb-2">Materiais aceitos:</h4>
